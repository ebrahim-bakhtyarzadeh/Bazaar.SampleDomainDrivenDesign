...
namespace Bazzar.Infrastructures.Data.SqlServer.Advertisments
{
	public class AdvertismentConfig : IEntityTypeConfiguration<Advertisment>
	{
		...
		{
			builder.Property(c => c.Price).IsRequired(false).HasConversion(c => c.Value.Value, d => Price.FromLong(d));
			builder.Property(c => c.OwnerId).HasConversion(c => c.Value.ToString(), d => UserId.FromString(d));
			builder.Property(c => c.ApprovedBy).IsRequired(false).HasConversion(c => c.Value.ToString(), d => UserId.FromString(d));
			builder.Property(c => c.Text).IsRequired(false).HasConversion(c => c.Value, d => AdvertismentText.FromString(d));
			builder.Property(c => c.Title).IsRequired(false).HasConversion(c => c.Value, d => AdvertismentTitle.FromString(d));
		}
	}
